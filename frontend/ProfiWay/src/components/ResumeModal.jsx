...
// Artık applicationId'yi ayrı prop olarak alıyoruz
export default function ResumeModal({ user, resume, onClose, onApprove, onReject }) {

  // Güvenli erişim için helper
  const getSafeValue = (obj, path, defaultValue = 'N/A') => {
    // user null ise veya path yoksa defaultValue dön
    if (!obj) return defaultValue;
    return path.split('.').reduce((acc, part) => acc && acc[part], obj) ?? defaultValue;
  };

  // Yetkinlikleri formatlı gösteren helper
  const renderCompetences = (competences) => {
    // --- API Yanıtına Göre Burayı Ayarlaman Gerekebilir! ---
    // console.log("Rendering competences:", competences); // Yapıyı görmek için log ekle

    if (!competences) return <p className="text-gray-600 text-sm">Yetkinlik bilgisi yok.</p>;

    // Varsayım 1: Dizi ve içinde competenceName var: [{id: 1, competenceName: 'React'}, ...]
      ...
      return (
        <ul className="list-disc list-inside text-gray-600 text-sm space-y-1">
          {competences.map((comp, index) => (
            <li key={comp.id || index}>{comp.competenceName}</li> // ID varsa key olarak kullan
          ))}
        </ul>
      );
    }
     // Varsayım 2: Dizi ve içinde name var: [{id: 1, name: 'React'}, ...]
    if (Array.isArray(competences) && competences.length > 0 && typeof competences[0] === 'object' && competences[0]?.name) {
      return (
        <ul className="list-disc list-inside text-gray-600 text-sm space-y-1">
          {competences.map((comp, index) => (
            <li key={comp.id || index}>{comp.name}</li> // ID varsa key olarak kullan
          ))}
        </ul>
      );
    }
    // Varsayım 3: Sadece string dizisi: ['React', 'Node']
  ...
  // resume null ise modal hiç render olmamalı ama yine de kontrol ekleyelim
    ...
    <div className="fixed inset-0 z-50 bg-black bg-opacity-60 flex items-center justify-center p-4 transition-opacity duration-300">
      <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto m-4 flex flex-col"> {/* Flex column */}
        ...
        <div className="flex justify-between items-center p-4 md:p-6 border-b sticky top-0 bg-white z-10">
          ...
          <button
            onClick={onClose}
            className="text-gray-500 hover:text-gray-700 transition-colors p-1 rounded-full hover:bg-gray-100" // Padding ve hover eklendi
            aria-label="Kapat"
          >
        ...
        {/* İçerik Alanı */}
        <div className="p-4 md:p-6 space-y-5 flex-grow"> {/* flex-grow eklendi */}
            {/* Kullanıcı Bilgileri */}
             <div className='mb-5'>
                <h3 className="text-lg font-semibold text-gray-700 mb-2 border-b pb-1">Başvuran Bilgileri</h3>
                {/* user objesi boş değilse ({}) bilgileri göster */}
        ...
        <div className="flex justify-end gap-4 p-4 border-t sticky bottom-0 bg-gray-50 z-10">
            <button
              onClick={onClose} // Kapat butonu da ekleyelim
